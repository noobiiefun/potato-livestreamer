...
/**
 * Engine relay: PC kirim JPEG + AAC -> HP encode H.264 -> push RTMP.
 */
class RtmpRelayEngine(
    ...
    private var videoBitrate = 3_000_000
        ...
        readThread = Thread {
            try {
                ...
                encoder = H264SurfaceEncoder(
                    ...
                    bitrate = videoBitrate, fps = videoFps,
    ...
    override fun onAuthError() = listener.onRelayFailed("Autentikasi RTMP gagal")
    ...
    private fun handleDroppedConnection(reason: String) {
        ...
        Thread {
            ...
            try { rtmpClient.disconnect() } catch (_: Exception) {}
            try { rtmpClient.connect(currentRtmpUrl) } catch (_: Exception) {}
        }.apply { isDaemon = true; start() }
    }
}
